...
/*
 * orderCurrencyUnitPriceMap = [:] if (purchaseOrder && facility) { if
 * (ownerAcctgPref) { ownerCurrencyUomId = ownerAcctgPref.baseCurrencyUomId
 * orderCurrencyUomId = purchaseOrder.currencyUom if
 * (!orderCurrencyUomId.equals(ownerCurrencyUomId)) { purchaseOrderItems.each {
 * item -> orderCurrencyUnitPriceMap.(item.orderItemSeqId) = item.unitPrice
 * serviceResults = runService('convertUom', [uomId : orderCurrencyUomId,
 * uomIdTo : ownerCurrencyUomId, originalValue : item.unitPrice]) if
 * (ServiceUtil.isError(serviceResults)) {
 * request.setAttribute("_ERROR_MESSAGE_",
 * ServiceUtil.getErrorMessage(serviceResults)) return } else { convertedValue =
 * serviceResults.convertedValue if (convertedValue) { item.unitPrice =
 * convertedValue } } } }
 * 
 * // put the pref currency in the map for display and form use
 * context.currencyUomId = ownerCurrencyUomId context.orderCurrencyUomId =
 * orderCurrencyUomId } else { request.setAttribute("_ERROR_MESSAGE_",
